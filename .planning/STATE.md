...
`hubspot_id` rather than re-upserting it. **`$hubspotUpdateMap` has no model-facing accessor yet**
— `SyncsToHubspot.php` is 04-04's file and the two plans share a wave, so 04-03 could not touch it;
no plan in the current roadmap (04-04 through 04-09) yet owns adding `getHubspotUpdateMap()` to the
trait (recorded in WINDOWS.md). Also fixed R3's architecture rule to admit the bare global function
...
- [Phase ?]: [Phase 4] 04-03: $hubspotUpdateMap has no model-facing accessor yet -- SyncsToHubspot.php is 04-04's file and the two plans share a parallel wave, so the job narrows to [] until a future plan adds getHubspotUpdateMap() to the trait; the selection rule itself is fully implemented and unit-tested in PropertyMapper::mapForUpdate()
